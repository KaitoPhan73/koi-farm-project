...
import { updateProductBase } from "@/apis/product-base";
...
interface FormUpdateProductBaseProps {
  initialData: TProductBaseRequest & { _id: string };
  categories: TCategoryResponse[];
}

export function FormUpdateProductBase({
  initialData,
  categories,
}: FormUpdateProductBaseProps) {
  ...
  const onSubmit = async (data: TProductBaseRequest) => {
    setIsLoading(true);
    try {
      const response = await updateProductBase(initialData._id, data);
      if (response.status === 200) {
        toast({
          title: "Product Base Updated Successfully",
        });
        router.refresh();
      }
    } catch (error) {
      toast({
        title: "Error",
        description: `Failed to update product base: ${error}`,
      });
    } finally {
      setIsLoading(false);
    }
